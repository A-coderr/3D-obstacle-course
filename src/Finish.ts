...
import finish from './assets/models/finish.glb'
import finish_img from './assets/img/finish.png'
...
export default class Finish {
  ...
  constructor(scene: Scene, world: World, position: [number, number, number]) {
    this.texture = new TextureLoader().load(finish_img, (texture) => {
      texture.repeat.x = 2
    ...
    new GLTFLoader().load(finish, (gltf) => {
